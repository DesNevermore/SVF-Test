...
private:
    ...
    // 以传过来的 Modification 对应文件的绝对地址为基础，为每一个 File 都维持一个 context，最后一起 dump 到文件
    ...
    // 统一为在原来的文件名后面加个 "_synthesized" 为新文件名，放在同路径上，e.g. "a.c" => "a_synthesized.c".
    FILE* outFp = nullptr;
...
// 下面函数用来设置 Modification 的状态，你可以想想这些有哪些需要存在类里面 field 里的，
    ...
    void addNewCodeSnippet(const SVFValue* startInst, const SVFValue* endInst, std::string str);
    ...
    /// HoleFilling.
    // 可以理解为 hole 就是 "$"" + "holeNumber"，比如 $1, $2, $3, ...，直接字符串精准匹配，换成 varName。
